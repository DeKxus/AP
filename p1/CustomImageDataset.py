...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.task == 'classification':
            ...
            label = read_image(os.path.join(self.img_dir, self.img_labels.iloc[idx, 0]+'mask'+self.img_labels.iloc[idx, 0][-4:]))
        else:
            raise ValueError('Task must be either "classification", or "multilabel_classification", or "semantic_segmentation"')
    ...
    def show_mask(self, idx):
        img_path = os.path.join(self.img_dir, self.img_labels.iloc[idx, 0]+'mask'+self.img_labels.iloc[idx, 0][-4:])
        image = read_image(img_path)
        plt.imshow(image.permute(1, 2, 0))
        plt.axis('off')
        plt.title(self.img_labels.iloc[idx, 1])
    ...
    def show_batch(self, images=None, labels=None, batch_size=4):
        ...
        for i in range(min(batch_size,grid_sz*grid_sz)):
            ...
            if self.task == 'classification' and images == None:
                ...
                ax[i//grid_sz,i%grid_sz].set_title(str(labels[i].item())+" : "+self.idx2class[labels[i].item()])
...
if __name__ == '__main__':
    ...
    # Load the test set
    test_dataset = CustomImageDataset(annotations_file='./superimposed_images/test.csv', img_dir='./superimposed_images/test/')
